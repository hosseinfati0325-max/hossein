...
class HuggingFaceProvider(AIProvider):
    ...
    def __init__(self, api_key: Optional[str] = None, model: Optional[str] = None, priority: int = 5):
        selected_model = model or settings.DEFAULT_HF_MODEL
        super().__init__(
            name="huggingface",
            priority=priority,
            is_free=True,
    ...
    async def health_check(self) -> bool:
        ...
        try:
            ...
            async with httpx.AsyncClient(timeout=5.0) as client:
                res = await client.get(f"https://huggingface.co/api/models/{self.model}", headers=headers)
    ...
    async def generate_json(
        ...
        messages: List[Dict[str, str]] = []
        sys_prompt = (system_instruction or "") + "\nRespond with valid raw JSON only. Do not include markdown code blocks."
